'''
Simula múltiples caminatas
Una sola caminata aleatoria es una cosa, pero eso no te dice si tienes una buena oportunidad de ganar la apuesta.

Para tener una idea de cuán grandes son sus posibilidades de alcanzar 60 pasos, puede simular repetidamente la caminata aleatoria y recopilar los resultados.
Eso es exactamente lo que harás en este ejercicio.

El código de muestra ya te pone en la dirección correcta.
Otro for bucle está envuelto alrededor del código que ya escribió.
Depende de usted agregar algunos fragmentos para asegurarse de que todos los resultados se registren correctamente.
...
for i in range(10) : # simulamos la caminata aleatoria 10 veces
